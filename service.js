...
!function (i) {
    ...
    e.prototype.autoPlay = function () {
        ...
        i.slideCount > i.options.slidesToShow && (
            i.autoPlayTimer = setInterval(i.autoPlayIterator, 2000)
        )
    },
...
window.onload = () => {
        ...
        const inpFormAgree = document.querySelector(".checkbox-check input");
        const inpSubmitForm = document.querySelector('.submit')
        const inpSubmitFormBgc = document.querySelector('.col-sm-12 .button')
        const changeOpacityBtnSubmit = () => {
            if(inpFormAgree.checked){
            inpSubmitForm.style.opacity = "1"
            inpSubmitFormBgc.style.opacity = "1"
            }else{
                inpSubmitForm.style.opacity = ".4"
                inpSubmitFormBgc.style.opacity = ".7"
            }
        }
        changeOpacityBtnSubmit()
        
        inpFormAgree.onchange = () => {
            changeOpacityBtnSubmit()
        }
        inpSubmitForm.onclick = (e) => {
            if(!inpFormAgree.checked){
                e.preventDefault()
                console.log(5158)
                return false
